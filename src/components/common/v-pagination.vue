...
export default {
  ...
  computed: {
    ...
    arrayButtons: function () {
      var quantity = 0
      if (this.localProducts.length % this.itemsOnPage > 0) {
        quantity = Math.floor(this.localProducts.length / this.itemsOnPage) + 1
      } else {
        quantity = this.localProducts.length / this.itemsOnPage
      }
      var arr = []
